...
Data available through kaggle at: https://www.kaggle.com/c/train495
Professors Analysis: 
...
```{r}
train <- train %>% select(-cast_total_facebook_likes, -movie_imdb_link, -facenumber_in_poster, -plot_keywords, -country, -movie_facebook_likes, -director_facebook_likes, -actor_1_name, -actor_2_name, -actor_3_name, -actor_1_facebook_likes, -actor_2_facebook_likes, -actor_3_facebook_likes, -cast_total_facebook_likes, -keyword_cat, -genres)
```
...
For my own personal model, I decided to run with a stochastic gradient boost, mainly jus because it was the model that I wanted to learn more about. So I ran through the model and did some tuning in order to hammer down my tuning paramter. This result gave me the best out of the previous two models, so this is the one I am rolling with for the remainder of this analysis.
```{r}
tune.grid = expand.grid(n.trees = 199,
                        interaction.depth = 6,
                        shrinkage = .09,
                        n.minobsinnode = 16)
...
write_csv(x=imdb.preds, path="./Peters_Submission_boost5_THISONE.csv")
```
