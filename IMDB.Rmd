...
Data for this model was part of an InClass competition hosted through Kaggle.
...
train <- train %>% select(-cast_total_facebook_likes, -movie_imdb_link, #-facenumber_in_poster, 
                          ...
                          -movie_facebook_likes, -director_facebook_likes, -actor_1_name, -actor_2_name, -actor_3_name, -actor_1_facebook_likes, -actor_2_facebook_likes, -actor_3_facebook_likes, -cast_total_facebook_likes, -keyword_cat, -genres)
```
...
Since we dont care about inference, we only care about predictability, we are going to go through some different transformations to better fit our data to our model. Dummy variables, and denter and scaling.

```{r}
#Dummary Vars
IVTrands <- dummyVars(imdb_score~.-movie_title-Set, data=train)
...
For my own personal model, I decided to run with a stochastic gradient boost, mainly just because it was the model that I wanted to learn more about. So I ran through the model and did some tuning in order to hammer down my tuning parameter. This result gave me the best out of the previous two models, so this is the one I am rolling with for the remainder of this analysis.
```{r}
tune.grid = expand.grid(n.trees = c(205, 206, 207),
                        interaction.depth = c(8, 9, 10),
                        shrinkage = c(.07, .08, .09),
                        n.minobsinnode = c(13, 14, 15))
...
boost$bestTune
boost$results
boost$results[rownames(boost$results) == 24,]
```
...
write_csv(x=imdb.preds, path="./holder_sub.csv")
```
...
```{r}
tune.grid = expand.grid(nrounds = seq(20, 50, 5),
                        lambda= seq(.05, .15, .05),
                        alpha=c(.75, .1),
                        eta=seq(.2, .4, .1))

boost1 <- train(form=imdb_score~., 
                data=(imdb.train %>% select(-Set, -movie_title)),
                method = "xgbTree",
                trControl=trainControl(method="repeatedcv",
                                       number=10,
...
```{r}
boost1$results[rownames(boost1$results) == 36,]$RMSE
```
